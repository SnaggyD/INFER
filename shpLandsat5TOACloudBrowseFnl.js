...
////////////// LANDSAT 5
var landsat5Toa = landsat5Toa.filterBounds(StudyArea)
    .filterDate('1984-03-01', '2012-06-01');

landsat5Toa = landsat5Toa.filter(ee.Filter.contains({ leftField: '.geo', rightValue: StudyArea }));

// Call the functions that were set up above.
var landsat5Toa = landsat5Toa.map(addCC);   // Add Cloud Cover percentage for whole image as a band.
var landsat5Toa = landsat5Toa.map(addSCS);  // Calculate the Simple Cloud Score for each pixel and add it as a band.
var landsat5Toa = landsat5Toa.map(addTime); // Add the image timestamp as a band .

print ('The results collection:',landsat5Toa);
...
var Collection = landsat5Toa.map(function(img) {
    var img_id = img.id();
...
var chrtData = landsat5Toa.select(['CLOUD_COVER_LAND','cloud'],['For Image','For ROI']);
...
var image = ee.Image(landsat5Toa.sort('CLOUD_COVER_LAND').first());
