...
var landsat4Toa = ee.ImageCollection('LANDSAT/LT04/C01/T1_TOA');
...
var strStudyAreaName = "Knockaderry Reservoir"
var StudyArea = table.filter(ee.Filter.eq("NAME","Knockaderry Reservoir")).geometry();
//var StudyArea = table.filter(ee.Filter.eq("EDENLakeCo","16001820070")).geometry();

//Filter early and improve performance. The date range refers to the operational life of satellite
var landsat4Toa = landsat4Toa.filterBounds(StudyArea)
    .filterDate('1982-08-01', '1993-12-31');
...
landsat4Toa = landsat4Toa.filter(ee.Filter.contains({ leftField: '.geo', rightValue: StudyArea }));

// Call the functions that were set up above.
var landsat4Toa = landsat4Toa.map(addCC);   // Add Cloud Cover percentage for whole image as a band.
var landsat4Toa = landsat4Toa.map(addSCS);  // Calculate the Simple Cloud Score for each pixel and add it as a band.
var landsat4Toa = landsat4Toa.map(addTime); // Add the image timestamp as a band .

print ('The results collection:',landsat4Toa);
...
var Collection = landsat4Toa.map(function(img) {
    var img_id = img.id();
...
var chrtData = landsat4Toa.select(['CLOUD_COVER_LAND','cloud'],['For Image','For ROI']);
...
var image = ee.Image(landsat4Toa.sort('CLOUD_COVER_LAND').first());
...
tempTimeSeries.onClick(function(xValue, yValue, seriesName) {
  ...
  var image = ee.Image(landsat4Toa.filter(equalDate).first());
