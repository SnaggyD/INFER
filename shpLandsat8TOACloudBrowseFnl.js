...
var landsat8Toa = landsat8Toa.map(addCC);   // Add Cloud Cover percentage for whole image as a band.
var landsat8Toa = landsat8Toa.map(addSCS);  // Calculate the Simple Cloud Score for each pixel and add it as a band.
var landsat8Toa = landsat8Toa.map(addTime); // Add the image timestamp as a band .

print ('The results collection:',landsat8Toa);
...
var Collection = landsat8Toa.map(function(img) {
    var img_id = img.id();
...
var chrtData = landsat8Toa.select(['CLOUD_COVER_LAND','cloud'],['For Image','For ROI']);
...
var image = ee.Image(landsat8Toa.sort('CLOUD_COVER_LAND').first());
